...
new_job(WorkerPid, JobID, Job) ->
    gen_server:cast(WorkerPid, {new_job, self(), JobID, Job}).
...
start_link(ManagerPid) ->
    gen_server:start_link(?MODULE, ManagerPid, []).
...
%% @end
%%--------------------------------------------------------------------
handle_cast({new_job, FromPID, JobID, Job}, State) ->
    Result = do_job(Job),
    FromPID ! {job_done, self(), JobID, Result},
    {noreply, State};
...
do_job(Job)->
    ?d("(~p) doing important job", [self()]),
    timer:sleep(2000),
    ?d("(~p) finished job", [self()]),
    Job*2.
